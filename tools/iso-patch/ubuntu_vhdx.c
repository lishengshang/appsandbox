...
static void plant_firstboot_service(ext4_writer_t *ew)
{
    ...
    const char *firstboot_sh =
        ...
        "\n"
        "# --- STEP 6.5: GNOME/Wayland keyboard layout ---\n"
        "# /etc/default/keyboard (STEP 4) is honored by console-setup and the\n"
        "# Xorg fallback only; the GNOME Wayland session reads gsettings\n"
        "# (org.gnome.desktop.input-sources) and defaults to 'us' otherwise —\n"
        "# which is why a German host still gets a US layout (issue #76).\n"
        "# Pre-seed the user's dconf before the first session; failures are\n"
        "# non-fatal (worst case the layout is picked in Settings).\n"
        "echo \"==== STEP 6.5: GNOME keyboard layout ====\"\n"
        "if [ -n \"$KB\" ] && id \"$ASB_USER\" >/dev/null 2>&1; then\n"
        "    if command -v dbus-run-session >/dev/null 2>&1 && command -v gsettings >/dev/null 2>&1; then\n"
        "        if su \"$ASB_USER\" -c \"dbus-run-session -- gsettings set org.gnome.desktop.input-sources sources \\\"[('xkb','$KB')]\\\"\" 2>/dev/null; then\n"
        "            su \"$ASB_USER\" -c \"dbus-run-session -- gsettings set org.gnome.desktop.input-sources mru-sources \\\"[('xkb','$KB')]\\\"\" 2>/dev/null || true\n"
        "            echo \"OK: GNOME input-source=$KB\"\n"
        "        else\n"
        "            echo \"WARN: gsettings input-source failed (ignored)\"\n"
        "        fi\n"
        "    else\n"
        "        echo \"SKIP: no dbus-run-session/gsettings\"\n"
        "    fi\n"
        "fi\n"
        "\n"
